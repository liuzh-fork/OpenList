...
1. 覆盖下面的源码，提交到 4.x 分支
2. 构建普通版本: `docker build -t abel533/openlist:v4.0.1 .`
3. 构建aio版本： `docker build --build-arg INSTALL_FFMPEG=true --build-arg INSTALL_ARIA2=true -t abel533/openlist:v4.0.1-aio .`
4. 登录 docker： `docker login -u 用户名`
5. 推送镜像： `docker push abel533/openlist:v4.0.1-aio`
...
```bash
docker run -d   --name openlist   -p 5244:5244   -p 5245:5245   -v ./data:/opt/openlist/data   abel533/openlist:v4.0.1-aio
```
