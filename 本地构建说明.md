...
1. 基于 tag 创建 4.x 分支：`git checkout -b 4.x v4.0.8`
2. 合并 quark 代码：`git rebase quark`
3. 构建普通版本: `docker build -t abel533/openlist:v4.0.8 .`
4. 构建aio版本： `docker build --build-arg INSTALL_FFMPEG=true --build-arg INSTALL_ARIA2=true -t abel533/openlist:v4.0.8-aio .`
5. 登录 docker： `docker login -u 用户名`
6. 推送镜像： `docker push abel533/openlist:v4.0.8-aio`
...
```bash
docker run -rm --name openlist -p 5244:5244 -p 5245:5245 abel533/openlist:v4.0.8-aio
```


## 源码

覆盖 `drivers\quark_uc\util.go` 下面的代码: 
